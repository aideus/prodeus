...
AnnealingQuery_ AnnealingQuery(const Expression &prog) { return AnnealingQuery_(&prog); }
GPQuery_ GPQuery(const Expression &prog) { return GPQuery_(&prog); }


Expression* AnnealingQuery_::reeval(Environment *pEnv, Expression *pPartner, double tp, bool bForceEval)
{
    double min_v = 1e+50;
    Expression *prog_best = NULL;
    AnnealingQuery_ *pEvaluated = this;
    if(bForceEval || !bEvaluated || v.isRnd()) {
        Expression *prog = children[0]->reeval(pEnv,
                                               NULL, // TODO?????
                                               tp, bForceEval);
        // double min_v = 1e+50;
#ifdef VERBOSE_EVAL
        ...
        for(int i = 0; i < 5000; i++) {
            ...
            for(int j = 0; j < 100; j++) {
                Expression *prog_ = prog->reeval(pEnv,
                                                 NULL, //TODO?????
...
bool cmp(Expression *p1, Expression *p2) {
...
Expression* GPQuery_::reeval(Environment *pEnv, Expression *pPartner, double tp, bool bForceEval)
{
    GPQuery_ *pEvaluated = this;
    int nPop = 300;
    vector<Expression *> pop;
//    if(bForceEval || !bEvaluated || v.isRnd()) {
    for(int i = 0; i < nPop; i++) {
        pop.push_back(children[0]->reeval(pEnv, NULL, 1., bForceEval));
    }
    for(int j = 0; j < 100; j++) {
